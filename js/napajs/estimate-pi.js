...
function run(points, batches) {
  ...
  return Promise.all(promises).then(values => {
    const aggregate = values.reduce((acc, result) => acc + result.value, 0);
...
const nrOfPoints = 100000000;
run(nrOfPoints, 1)
  .then(result => run(nrOfPoints, 2))
  .then(result => run(nrOfPoints, 4))
  .then(result => run(nrOfPoints, 8))
  .then(result => run(nrOfPoints, 16))
  .then(result => run(nrOfPoints, 32))
  .then(result => run(nrOfPoints, 64))
