// Verallgemeinerte Ground-Truth-Extraktion fuer ALLE Formulare in Handlisten (nicht nur die
// 291...Kadmosbearbeitet-Datensaetze): jede .att-Datei enthaelt pro Zeilenpaar im Bild einen
// 12-stelligen Identcode (S=...), bei dem unsicher erkannte Ziffern als "+" markiert sind. Der
// Identcode-Pruefziffernalgorithmus (Gewichtung 4/9 alternierend von rechts, Pruefziffer =
// (10 - Summe mod 10) mod 10) legt eine EINZELNE unbekannte Ziffer eindeutig fest. Bei mehreren
// unbekannten Ziffern in einem Feld bleiben nur die bekannten Positionen nutzbar - es wird
// nichts geraten.
//
// Schreibt in jede .att-Zeile ein zusaetzliches K=-Feld (bekannte/aufgeloeste Ziffern, "+" wo
// weiterhin unbekannt) und extrahiert - wenn die Zeilenzahl im Bild zur Feldanzahl passt -
// fuer jede bekannte Ziffernposition einen Crop nach data/imgs/eb/test/img<digit>.
//
// Aufruf: node scripts/import/extract-ground-truth-digits.js [--root <Handlisten-Ordner>]
//         [--limit n] [--dry-run]
...
const logFile = path.join(dataPath, 'imgs', 'eb', 'ground-truth-extract.log');

// An mehreren Beispielen (2, 4, 10, 16 Zeilen) verifiziert - deckt die Ziffern-Zeilenbloecke
// aller bisher gesehenen Formularvarianten ab.
...
const tifToPng = (ffmpeg, tifFile) => {
  const outFile = path.join(os.tmpdir(), `gtx-${crypto.randomBytes(8).toString('hex')}.png`);
  execFileSync(ffmpeg, ['-y', '-i', tifFile, '-update', '1', '-frames:v', '1', outFile], { stdio: 'ignore' });
  return outFile;
};

// img.js exponiert Breite/Hoehe nicht in seiner oeffentlichen API, daher wird hier
// durchgehend mit (imgdata, w, h) statt mit dem img.js-Objekt selbst gearbeitet.
...
// Identcode-Pruefziffer: Gewichtung 4/9 alternierend von rechts ueber die ersten 11 Ziffern.
...
// Gibt ein Array aus 12 Eintraegen zurueck: Ziffer (0-9) wo bekannt, sonst null. Bei genau
// einer unbekannten Ziffer wird sie ueber die Pruefziffer eindeutig aufgeloest.
const resolveField = (raw) => {
  ...
  if (unknownIdx.length === 0) return base;
...
const parseFieldLines = (attContent) =>
  attContent
    .split('\n')
...
const main = () => {
  ...
  const stats = {
    forms: 0,
    skippedRowMismatch: 0,
    skippedOther: 0,
    fieldsTotal: 0,
    fieldsFullyKnown: 0,
    digitsKnown: 0,
    digitsExtracted: 0,
    digitsWritten: Array(10).fill(0),
  };

  toProcess.forEach((attName) => {
    const attFile = path.join(ROOT, attName);
    const tifFile = attFile.replace(/\.att$/i, '.tif').replace(/\.ATT$/, '.TIF');
    let pngFile;
    try {
      const content = fs.readFileSync(attFile, 'utf8');
      const fieldLines = parseFieldLines(content);
      if (fieldLines.length === 0) throw new Error('keine Identcode-Felder in .att');
      stats.fieldsTotal += fieldLines.length;

      const resolvedByField = fieldLines.map(({ m }) => resolveField(m[1]));
      let newContent = content;
      fieldLines.forEach(({ line }, fieldIdx) => {
        const digits = resolvedByField[fieldIdx];
        const known = digits.filter((d) => d !== null).length;
        stats.digitsKnown += known;
        if (known === FIELD_LEN) stats.fieldsFullyKnown++;
        const kValue = digits.map((d) => (d === null ? '+' : d)).join('');
        const cleanLine = line.replace(/;?\s*K=[0-9+]*;?/, ';').replace(/;?\s*$/, '');
        const newLine = `${cleanLine};K=${kValue};`;
        newContent = newContent.replace(line, newLine);
      });
      if (!DRY_RUN) fs.writeFileSync(attFile, newContent);
      ...
      if (rows.length !== fieldLines.length * 2) {
        stats.skippedRowMismatch++;
        if (!DRY_RUN) appendLog(attName);
        return;
      }

      fieldLines.forEach((_, fieldIdx) => {
        const digits = resolvedByField[fieldIdx];
      ...
      console.error(`Fehler bei ${attName}: ${error.message}`);
      stats.skippedOther++;
    } finally {
