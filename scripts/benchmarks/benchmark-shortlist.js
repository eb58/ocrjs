...
if (!isMainThread) {
  ...
  const predictions = tasks.map(({ file, expected, index }) => ({
    index,
    prediction: analyzeImage(file, expected, dataset, databases, 2.4, { candidateLimit }).prediction,
  }));
  ...
  const outputName = requested.length
    ? `benchmark-shortlist-${requested.join('-')}-results.json`
    : 'benchmark-shortlist-results.json';
