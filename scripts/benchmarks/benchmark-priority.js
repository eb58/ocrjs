...
if (!isMainThread) {
  ...
  parentPort.postMessage(
    tasks.map(({ file, expected, index }) => ({
      index,
      result: analyzeImage(file, expected, dataset, databases, 2.4, { priorityCount }),
    })),
  );
} else {
  ...
  const run = async () => {
    for (const dataset of ['eb', 'mnist']) {
      ...
      for (const priorityCount of [0, 32]) {
        ...
        const changed = tasks
          .filter((_, index) => JSON.stringify(results[index]) !== JSON.stringify(baseline[index]))
          .map(({ file }) => file);
        const row = {
          dataset,
          priorityCount,
