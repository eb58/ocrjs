...
if (!isMainThread) {
  ...
  const results = workerData.tasks.map(({ file, expected, index }) => {
    ...
    const full =
      fast.confidence < thresholds.at(-1) ? analyzeImage(file, expected, workerData.dataset, databases) : undefined;
  ...
  const run = async (dataset) => {
        ...
        (_, chunk) =>
          new Promise((resolve, reject) => {
            const worker = new Worker(__filename, {
              workerData: {
                dataset,
                tasks: tasks.filter(({ index }) => index % workers === chunk),
              },
            });
    ...
    const variants = thresholds.map((threshold) => {
      ...
      return {
        ...
        fixedFastErrors: results.filter(
          (result, index) => result.fast !== tasks[index].expected && predictions[index] === tasks[index].expected,
        ).length,
        regressedFastHits: results.filter(
          (result, index) => result.fast === tasks[index].expected && predictions[index] !== tasks[index].expected,
        ).length,
      };
    });
    return {
      dataset,
      total: tasks.length,
      analysisSeconds: Math.round((performance.now() - started) / 100) / 10,
      variants,
    };
  };
  Promise.all(['eb', 'mnist'].map(run))
    .then((results) => {
      const report = { created: new Date().toISOString(), workers, results };
      fs.writeFileSync(
        path.join(__dirname, 'benchmark-shortlist-fallback-results.json'),
